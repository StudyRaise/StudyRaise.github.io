...
var PDFViewerApplication = {
  ...
  download: function download() {
    ...
    try {
      var pdfInfoStr = window.localStorage.getItem('pdfinfo');
      if (pdfInfoStr) {
        var pdfInfo = JSON.parse(pdfInfoStr);
        pdfInfo.printstate = '下载';
        var xhr = new XMLHttpRequest();
        xhr.open('post', 'http://127.0.0.1:9000/SimpleChat/pdf/add');
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send(urlEncode(pdfInfo));
